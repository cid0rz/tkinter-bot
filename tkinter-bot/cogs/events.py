...
class Events(commands.Cog):
    ...
    async def on_ready(self):
        ...
        self.guild = self.bot.get_guild(messages.GUILD_ID)
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        await member.send(f'Hope you enjoy your stay, make sure to read the <#840634593790132243> channel and follow the rules!')
        await member.send(file=discord.File(fp=img,filename='welcome.png'))

        await member.add_roles(role)
        await member.send(f'You have been promoted to the role of a @New Member in {self.guild.name}')
    ...
    async def check_new_members(self):
        ...
        if days >= 3 and not is_bot:
            ...
            await member.send(f'Congratulations, you have been promoted to the role of a @Member in {self.guild.name}')
    ...
    async def add_offline_roles(self):
        ...
        for member in all_mem:
            ...
            if days < 3 and member not in new_mem.members and not is_bot:
                await member.add_roles(new_mem, reason='Could not give the role due to bot maintanence, so now given.')
                await member.send(f'Sorry, we could not give you this role earlier but you now have been promoted to the role of a @New Member in {self.guild.name}')

    @tasks.loop(seconds=20)
    async def presence(self):
        users = len([member async for member in self.guild.fetch_members() if not member.bot])
        task = next(self.activities)
        await self.bot.change_presence(activity=discord.Activity(buttons=['list1']))
